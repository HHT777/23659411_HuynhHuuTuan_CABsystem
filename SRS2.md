...
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------- | ----------------------------------------------- |
| UC-01 | POST `/api/v1/auth/register`                                                                                                                                      | phone,name,password                             | 201 User; 409 phone trùng                       |
...
| UC-03 | GET/PATCH `/api/v1/me/profile`                                                                                                                                    | PATCH name,phone                                | 200 Profile; 409 phone trùng                    |
| UC-04 | PATCH `/api/v1/drivers/me/availability`                                                                                                                           | availability                                    | 200 Driver; 409 đang có chuyến                  |
| UC-05 | POST `/api/v1/ride-requests`; GET `/api/v1/ride-requests/{id}`                                                                                                    | pickup,destination,vehicleType; Idempotency-Key | 201 RideRequest / 200; 409 yêu cầu đang mở      |
...
| UC-13 | GET `/api/v1/notifications`; PATCH `/api/v1/notifications/{id}`                                                                                                   | PATCH read=true                                 | 200 list/Notification; 404 khác chủ             |
...
| UC-16 | GET `/api/v1/operations/drivers`; PATCH `/api/v1/operations/drivers/{id}`; PATCH `/api/v1/admin/roles/{id}`; POST `/api/v1/admin/fares`                           | status/role/biểu giá                            | 200/201; 403 ngoài quyền                        |
...
SRS giữ nguyên chuỗi phân tích 02–18, đồng thời có baseline tại mục 05.IV, 08.V, 10.I.a, 12.9, 16.4 và 18.9. Với các bảng cũ ghi `\[NEED CLARIFICATION]` hoặc `\[Cần làm rõ]` ở cùng chủ đề, chính sách định lượng ở các mục trên là đầu vào **đã chọn bởi nhóm dự án** cho API document và test case. Chỉ contract callback của nhà cung cấp thanh toán thực tế và phê duyệt thương mại của Công ty ABC chưa có; bản thí điểm dùng mock/sandbox. Khi làm API/test, dẫn đồng thời BR → FR → Rule/Exception → UC → AC → BC.
